...
That on how many users connected is a bit faulty on how many videos(correct now) send a text message to see if ok the message sent to server and returns broadcast-enough to test if not returning not connected(to appear in conversation)
